...
import com.example.demo.apiFormats.TreeResponseFormat;
...
//	@GetMapping(value="/familytree/{houseName}")
	public ResponseEntity<List<CharacterWithChildrenResponse>> getCharactersForGivenHouse(@PathVariable("houseName") String houseName){
		log.info("in controller :: getting all characters for the given house name : {}", houseName);
		
		try {
			
			List<CharacterWithChildrenResponse> allCharsForGivenHouse = this.charService.getCharactersByHouseName(houseName);
	...
	@GetMapping(value="/familytree/{houseName}")
	public ResponseEntity<TreeResponseFormat> getCharactersForGivenHouseInReactTreeFormat(@PathVariable("houseName") String houseName){
		log.info("in controller :: getting all characters for the given house name : {}", houseName);
		
		try {
			
			List<CharacterWithChildrenResponse> allCharsForGivenHouse = this.charService.getCharactersByHouseName(houseName);
			log.info("ITEMS IN allCharsForGivenHouse :: {}", allCharsForGivenHouse.size());
			
			TreeResponseFormat output = new TreeResponseFormat();
			output.setName(houseName);
			output.setAttributes(null);
			output.setChildren(allCharsForGivenHouse);
	...
	public ResponseEntity<List<CharacterResponse>> getAllFavCharacters(){
		log.info("in controller :: getting all the favourite characters");
		
		try {
			List<CharacterResponse> allFavChars = this.charService.findFavouriteCharacters();
			
			return ResponseEntity.status(HttpStatus.OK).body(allFavChars);
			
		}catch(Exception e) {
			e.printStackTrace();
			log.error("exception in getting favourites");
		}
		
		log.warn("no favourites received");
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
	}
